#!/usr/bin/env python3
"""End-to-end Python bridge check: calculate 1 + 1 through the daemon."""
...
def main() -> int:
    ...
    with DaemonManager() as manager:
        ...
        click_button(manager, actions, {"1", "One"})
        click_button(manager, actions, {"=", "Equals"})
        time.sleep(0.5)

    display = calculator_display_value()
    if display != "2":
        raise AssertionError(f"Expected Calculator display to be 2 after 1 + 1 =, got {display!r}.")

    print(json.dumps({"ok": True, "expression": "1+1", "display": display}, sort_keys=True))
    return 0
